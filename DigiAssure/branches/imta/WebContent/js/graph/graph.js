...
var Graph = Graph || (function($) {
	...
	Private.design = function(obj) {
		...
		styling.style[this.parseS(obj.id, '.id0')] = {
				"stroke": self.borderColor || "#fff",
				"stroke-width": self.borderWidth || "2",
				'fill': 'green',
				'opacity': 0.8
			};
		styling.style[this.parseS(obj.id, '.id1')] = {
				"stroke": self.borderColor || "#fff",
				"stroke-width": self.borderWidth || "2",
				'fill': 'red',
				'opacity': 0.8
			};
		styling.style[this.parseS(obj.id, '.id2')] = {
				"stroke": self.borderColor || "#fff",
				"stroke-width": self.borderWidth || "2",
				'fill': 'Beige',
				'opacity': 0.8
	...
	GraphBar.prototype.init = function(thing) {
		...
		if (!self.multipleDataSets) {
			for (var i = 0, len = xLines - 1; i < len; ++i) {
				...
				x = (i * self.xDist + self.mainOffset);
				//bars
				E.rects += '<rect class="rect bar id'+i+'" id="' + self.id + '-point-' + i + '" x="' + x +
...
var GraphTable = GraphTable || (function() {
	...
	GraphTable.prototype.init = function(thing) {
		...
		var table = '<table class="table table-striped table-bordered "id="' + self.id + '" border="1"cellpadding="5"><tr><th>#</th><th>' +
	...
	$.fn.graphify = $.fn.graphify || function(options) {
		...
		var buttons = (function() {
			...
			for (var i = 0, len = types.length; i < len; ++i) {
				btns += '<button id="' + id + '-graphify-button-' + types[i] + '" class="btn btn-default" >' +
					types[i].charAt(0).toUpperCase() + types[i].substring(1) +
					'</button>&emsp;';
			}
			return btns;
		})();
		if (opts.pos === 'top') $('#' + wrapper).prepend(buttons + '<br/><br /> </center>');
