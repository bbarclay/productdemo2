...
function drawGrapgs(appId){
	...
	//topics(appId);
	/* getEmotions(appId);  */
	ratingTimeLine(appId);
	reviews(appId);
	recommendation(appId);
...
function setAppName(appNameId){
	...
	var ratingObj = document.getElementById("ratingApp");
	var recommendationObj=document.getElementById("recommendationApp");
...
/* 	emotion.innerHTML=appName; */
	...
	ratingObj.innerHTML=appName;
	recommendationObj.innerHTML=appName;
}
...
/* 	var allDivs=['sentiment','words','sentimentTimeline','topicsDiv','review','userReviews']; */
  	var allDivs=['sentiment','words','sentimentTimeline','review','userReviews','recommendationDiv'];
...
/* 	var navMenus = ['sentimentNav','wordsNav','sentimentTimelineNav','topicsDivNav','emotionNav','reviewNav']; */
	...
	var navMenus = ['sentimentNav','wordsNav','sentimentTimelineNav','reviewNav','recommendationDivNav'];
	...
	$.ajax({
		...
		success : function(data) {
			...
			total= negative+positive+neutral;
				...
				//document.getElementById("sentimentValue").innerHTML=Number(Math.round(value));
			 	value=value*2.9; 
	...
	.ajax({
		...
		success : function(data) {
			...
			for (var i = 0; i < wordArrName.length; i++) {
				...
				percentagePositive
				.push(Number((wordArrName[i]["positive"] / totalPercentage[i]) * 100));
				percentageNegative
				.push(Number((wordArrName[i]["negative"] / totalPercentage[i]) * 100));
				percentageNeutral
				.push(Number((wordArrName[i]["neutral"] / totalPercentage[i]) * 100));
			}
			if (data) {
				var len = word.length;
				...
				if (len > 0) {
					for (var i = 0; i < len; i++) {
						var total=parseInt( negative[i])+parseInt( positive[i])+parseInt( neutral[i]);
						$("#popularWords")
						.append(
								...
								+ Number(Math.round(percentagePositive[i]))
								+ "%\nNeutral: "
								+ Number(Math.round(percentageNeutral[i]))
								+ "%\nNegative: "
								+ Number(Math.round(percentageNegative[i]))
...
function ratingTimeLine(appId){

	$
	.ajax({
		type : "GET",
		
		url : "${pageContext.request.contextPath}/digitalQAService/userExperienceTesting/sentimentAnalysis/ratingTimeline?appId="+appId,
		contentType : "application/json; charset=utf-8",
		dataType : "json",
		success : function(data) {
			...
			for (var i = 0; i < sentimentTimeline.length; i++) {
				date.push(sentimentTimeline[i]["Date"]);
				star1.push(parseInt(sentimentTimeline[i]["1 Star"]));
				star2.push(parseInt(sentimentTimeline[i]["2 Star"]));
				star3.push(parseInt(sentimentTimeline[i]["3 Star"]));
				star4.push(parseInt(sentimentTimeline[i]["4 Star"]));
				star5.push(parseInt(sentimentTimeline[i]["5 Star"]));
			}
...
function reviews(appId) {
	...
	$.ajax({
		...
		//var rates={"0":"0%","0.5":"10%","1":"20%","1.5":"30%"}	;
			...
			var sum = breakdown["5"]+breakdown["4"]+breakdown["3"]+breakdown["2"]+breakdown["1"];
			var rating1avg = breakdown["1"]/sum*100;
			var rating2avg = breakdown["2"]/sum*100;
			var rating3avg = breakdown["3"]/sum*100;
			var rating4avg = breakdown["4"]/sum*100;
			var rating5avg = breakdown["5"]/sum*100;
			$(".rating1").css("width",rating1avg+"%");
			$(".rating2").css("width",rating2avg+"%");
			$(".rating3").css("width",rating3avg+"%");
			$(".rating4").css("width",rating4avg+"%");
			$(".rating5").css("width",rating5avg+"%"); 
...
function recommendation(appId){
	$.ajax({
		type : "GET",

		url : "${pageContext.request.contextPath}/digitalQAService/userExperienceTesting/sentimentAnalysis/recommendation?appId="+appId,
		contentType : "application/json; charset=utf-8",
		dataType : "json",
		success : function(data) {
			...
			for(var i=0;i<observations.length;i++){
				htmlContent+="<h4><span class='	glyphicon glyphicon-check' style=' color: #5CB85C'></span> <nbsp>";
				htmlContent+=observations[i].observation+"</h4><br>";
			}
			document.getElementById("observations").innerHTML=htmlContent;
			
			var recommendations=data.recommendations;
			 htmlContent="";
			for(var i=0;i<recommendations.length;i++){
				htmlContent+="<h4><span class='	glyphicon glyphicon-check' style=' color: #5CB85C'></span> <nbsp>";
				htmlContent+=recommendations[i].recommendation+"</h4><br>";
			}
			document.getElementById("recommendationList").innerHTML=htmlContent;
...
<body onload="abcde()" class="newhome">
		...
		<div class="row">
			...
			<ul class="nav nav-tabs marginLeft15px">
				...
				<li id="sentimentTimelineNav"><a href="#"
					...
					onclick="return hideOther('sentimentTimeline')"> <b><button
								class="btn mobileLabMenu">Sentiment Timeline</button></b>

				</a></li>
				<li id="recommendationDivNav"><a href="#"
					title="Observation and Recommendation"
					onclick="return hideOther('recommendationDiv')" > <b><button
								class="btn mobileLabMenu">Observation & Recommendation</button></b>

				</a></li>
				<!-- <li id="ratingTimelineNav"><a href="#"
					title="Rating Timeline"
					onclick="return hideOther('ratingTimeline')" class=""> <b><button
								class="btn mobileLabMenu">Rating Timeline</button></b>
	...
	<div class="row marginLeftRight">
		<div class="col-sm-12 col-md-12 nodisplay" id="review">
			...
			<div class="greyTextColor">
					<h3 class="marginTop15px">
						<span id="ratingApp"></span> - Rating Timeline
					</h3>
				</div>
				<div class="marginTop15px"></div>
				<div id="container2" class="sentiment_timeline_div marginLeft15px"></div>
		...
		<div class="col-sm-12 col-md-12 nodisplay" id="sentiment">
			...
			<div
				...
				<div class="integratedQALabPanel">
					...
					<!-- <div  class="labelForGraph">Sentiment Score: <span id="sentimentValue"></span></div> -->
	...
	<div class="row marginLeftRight">
			<div id="recommendationDiv" class="col-sm-12 col-md-12 nodisplay">
				<div class="greyTextColor">
					<h3 class="txtCenter marginTop15px">
						<span id="recommendationApp"></span> - Sentiment Timeline
					</h3>
				</div>
				<div class="marginTop15px"></div>
				<h3>Observations</h3>
				<div id="observations" style="font: 15px;"></div>
				<br>
				<br>
				<h3>Recommendations</h3>
				<div id="recommendationList" style="font: 15px;">
